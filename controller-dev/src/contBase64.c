...
void longToBase64(uint64_t source, char* dest){
	for(int i = 0; i < LONG_TO_BASE64_LENGTH; i++){
		...
		*(dest+i) = base64_table[(source>>(60-6*i))%64];
	}
...
void byteArrToBase64(uint8_t* arr, uint64_t arrLen, char* dest){
	switch(arrLen%3){
		case 2: //with two characters, we use three digits
			...
			*(dest) = base64_table[arr[0]>>4];
			*(dest+1) = base64_table[((arr[0]%(1<<4))<<2) + (arr[1]>>6)];
			*(dest+2) = base64_table[arr[1]%(1<<6)];
			dest += 3;
			arrLen -= 2;
			arr += 2;
			break;
		case 1: //with one character, we use two digits for that character
			// 000012 | 34568
			*(dest) = base64_table[(arr[0]>>6)];
			*(dest+1) = base64_table[(arr[0]%(1<<6))];
			dest += 2;
			arrLen--;
			arr++;
			break;
	...
	for(int i = 0; i < 4*arrLen/3; i += 4){
		...
		*(dest+i) = base64_table[(arr[i]>>2)];
		*(dest+i+1) = base64_table[((arr[i]%(1<<2))<<6) + (arr[i+1]>>4)];
		*(dest+i+2) = base64_table[(arr[i+1]%(1<<4))+(arr[i+2]>>6)];
		*(dest+i+3) = base64_table[arr[i+2]%(1<<6)];
	}
